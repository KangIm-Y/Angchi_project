...
class BlueRatioCirculator(Node):
    ...
    def __init__(self):
        ...
        self.complement_group = MutuallyExclusiveCallbackGroup()
        ...
        self.joy_subscriber = self.create_subscription(
            ...
            qos_profile,
            callback_group=self.complement_group)
        ...
        self.angle_threshold = 25. #degree
        ##################
        ...
        self.roll_angle = 0.
        self.resized = np.zeros((self.img_size_y,self.img_size_x, 3), dtype = np.uint8)
        ...
        self.image_timer = self.create_timer(1/24, self.image_capture, callback_group=self.video_group)
        self.pub_timer = self.create_timer(1/24, self.publish_timer, callback_group=self.complement_group)
        
        self.get_logger().info('BlueRatioCirculator node has been initialized')
    ...
    def image_capture(self):
        ...
        if not ret :
            ...
            if self.joy_status == True :
            ...
            elif (self.roll_angle <= -self.angle_threshold) : ## minus is left
                if (L_sum <= R_sum*1.2) :
                    self.L_joy = (self.max_speed * (R_sum*1.2/(R_sum*1.2+L_sum)))
                    self.R_joy = (self.max_speed * (L_sum/(R_sum*1.2+L_sum)))
                elif (L_sum >= R_sum) :
                    self.L_joy = (self.max_speed * (R_sum/(R_sum+L_sum*1.2)))
                    self.R_joy = (self.max_speed * (L_sum*1.2/(R_sum+L_sum*1.2)))
                    
            elif (self.roll_angle >= self.angle_threshold)  : ## plus is right
                if (R_sum <= L_sum*1.2) :
                    self.L_joy = (self.max_speed * (R_sum/(R_sum+L_sum*1.2)))
                    self.R_joy = (self.max_speed * (L_sum*1.2/(R_sum+L_sum*1.2)))
                elif (R_sum >= L_sum) :
                ...
                self.L_joy = (self.max_speed / 1.25 ) * (0.25 if L_sum > R_sum else 1.)
                self.R_joy = (self.max_speed / 1.25 ) * (0.25 if L_sum < R_sum else 1.)
            elif ((L_sum > R_sum) | (R_sum > L_sum)) :
                self.L_joy = (self.max_speed * (R_sum/(R_sum+L_sum)))
                self.R_joy = (self.max_speed * (L_sum/(R_sum+L_sum)))
            else :
    ...
    def imu_updater(self, msg) :
        imu_data = msg.data
        self.roll_angle = imu_data[0]
