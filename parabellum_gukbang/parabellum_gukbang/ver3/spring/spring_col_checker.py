...
class SpringColorChecker(Node):
    def __init__(self):
        ...
        self.roll_check = self.create_timer(1/15, self.roll_finder)
        ...
        if self.joy_status == True :
            ...
            # turn right        
                ...
                if ((self.L_sum < (self.ROI_half_size * self.slant_drive_max)) & (self.L_sum > (self.ROI_half_size * self.slant_drive_min))) :
                    ...
                    self.L_joy = (self.max_speed / 2) - 2 
                    self.R_joy = (self.max_speed / 2) - 6
                    self.get_logger().info(f'right')
                else :
                    self.L_joy = self.before_L_joy
                    self.R_joy = self.before_R_joy
    ...
    def imu_msg_sampling(self, msg) :
        ...
        if (imu_data[0] <= 80) or (self.ahead_roll == -1) :
            self.robot_roll = -1
        elif (imu_data[0] >= 100) or (self.ahead_roll == 1) :
            self.robot_roll = 1
        else :
            self.robot_roll = 0
