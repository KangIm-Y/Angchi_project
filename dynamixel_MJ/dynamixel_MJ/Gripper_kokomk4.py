...
#from interfaces_gripper.msg import Age
...
#----------------------------- debug data ------------------------  
    ...
    def grip_callback(self, request, response):
        grip = request.data
        angle_step = 33
        initial_position = int(380 / 0.088)
        final_position = int(250/ 0.088)
...
#           while(1)
            ...
            elif self.current == 0: #Shutdown ERROR 걸리는 경우
                response.success = False
                response.message = 'Shutdown ERROR'
                self.get_logger().info(f'Present current: {self.current}')
                self.gripper(initial_position)          
                #break
            
                # elif self.current >= 4200000000: #역전류 흐르는 경우
                #     self.current = 0
                #     response.success = False
